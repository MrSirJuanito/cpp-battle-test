...
TEST(UnitSpawnTest, SpawnSwordsMan) {
    ...
    world.addUnit(std::shared_ptr<IUnit>(new SwordsManUnit(world, 1, 2)));
...
TEST(UnitSpawnTest, EventSpawnSwordsMan) {
    ...
    world.addUnit(std::shared_ptr<IUnit>(new SwordsManUnit(world, x0, y0)));
    std::string output = testing::internal::GetCapturedStdout();

    std::stringstream expected;
    expected << "[0] UNIT_SPAWNED unitId=0 unitType=Swordsman x=" << x0 << " y=" << y0 << " \n";
    EXPECT_EQ(output, expected.str());
}

TEST(UnitSpawnTest, SpawnHunter) {
    GameWorld world(10, 10);
    world.addUnit(std::shared_ptr<IUnit>(new HunterUnit(world, 1, 2)));
...
TEST(UnitSpawnTest, EventSpawnHunter) {
    ...
    std::stringstream expected;
    expected << "[0] UNIT_SPAWNED unitId=0 unitType=Hunter x=" << x0 << " y=" << y0 << " \n";
    EXPECT_EQ(output, expected.str());
}
...
TEST(UnitSpawnTest, MultipleUnitsSamePosition) {
    ...
    world.addUnit(std::shared_ptr<IUnit>(new HunterUnit(world, 0, 0)));
    EXPECT_ANY_THROW(world.addUnit(std::shared_ptr<IUnit>(new SwordsManUnit(world, 0, 0))));
}
...
TEST(UnitSpawnTest, MultipleUnitsCheckIds) {
    ...
    world.addUnit(std::shared_ptr<IUnit>(new HunterUnit(world, 0, 0)));
    world.addUnit(std::shared_ptr<IUnit>(new SwordsManUnit(world, 0, 1)));
    world.addUnit(std::shared_ptr<IUnit>(new HunterUnit(world, 1, 1)));
